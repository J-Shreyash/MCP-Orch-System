        ...
        if message_value and isinstance(message_value, str) and '|||MSG' in message_value:
            try:
                ...
                if text and unique_key != st.session_state.last_processed:
                    ...
                    is_csv_query = False
                    csv_keywords = ['show', 'query', 'select', 'count', 'display', 'list', 'top', 'bottom', 'chart', 'plot', 'graph', 'visualize']
                    
                    if st.session_state.current_table and any(kw in text.lower() for kw in csv_keywords):
                        is_csv_query = True
                    ...
                    with st.spinner("🤔 Processing..."):
                        try:
                            if is_csv_query:
                                ...
                                # Regular AI query
                                response = st.session_state.agent.process_query(text)
